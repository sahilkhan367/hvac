...
@app.get("/api/status/bulk")
def get_bulk_device_data(
    background_tasks: BackgroundTasks,
    slave_id: List[int] = Query(...),
    on: List[int] = Query(...),
    temp: List[int] = Query(...),
    speed: List[int] = Query(...)
):
    background_tasks.add_task(fetch_bulk_data, slave_id, on, temp, speed)
    return {"message": "Bulk fetch started", "slave_ids": slave_id}
